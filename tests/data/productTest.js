...
describe('test suite: testing class Product', () => {
  let product;
  ...
  it('check for correct property values', () => {
  ...
  it('check getting the stars url method', () => {
    expect(product.getStarsUrl()).toEqual(`images/ratings/rating-${product.rating.stars * 10}.png`);
  });

  it('check for getting right price', () => {
    expect(product.getPrice()).toEqual('$10.90');
  });

  it('check for extra info', () => {
    expect(product.extraInfoHTML()).toEqual('');
  });
});

describe('test suite: testing class Clothing', () => {
  let clothing;
  ...
  it('check size chart link', () => {
    expect(clothing.sizeChartLink).toEqual('images/clothing-size-chart.png');
  });

  it('check for extra info', () => {
    expect(clothing.extraInfoHTML()).toContain(`<a href="${clothing.sizeChartLink}" target="_blank">`);
    expect(clothing.extraInfoHTML()).toContain('Size Chart');
  });
});

describe('test suite: testing class Appliance', () => {
  let appliance;
  ...
  it('check for instructions and warranty link', () => {
